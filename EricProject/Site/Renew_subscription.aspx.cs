...
namespace EricProject.Site
{
    public partial class Renew_subscription : System.Web.UI.Page
    {
        ...
        protected void btncontinue_Click(object sender, EventArgs e)
        {
                ...
                while (drCheckCampainsActiveInactiveByMerchantID.Read())
                {
                    if (drCheckCampainsActiveInactiveByMerchantID["Exists"].ToString() == "1")
                    {
                        ...
                        else
                        {
                            ...
                            lblSubscriptionFailed.Visible = true;
                        }
                    }
                    else
                    {
            ...
            //        _Merchant objmerch = new _Merchant();
            //        objmerch.Merchant_ID = Convert.ToInt32(Session["MerchantId"].ToString());
            //        DAL.Plugin objplugin = new DAL.Plugin();
            //        int i = objplugin.ExtendMerchantPaidPeriodByMerchantID(objmerch);

            //        //  obj.ExtendMerchantPaidPeriodByMerchantID("ExtendMerchantPaidPeriodByMerchantID", Merchant_Id, Paid_period_expiry_date); 

            //        _Merchant objmerchant = new _Merchant();
            //        objmerchant.Merchant_ID = Convert.ToInt32(Session["MerchantId"].ToString());
            //        objmerchant.status = 1;
            //        int resultmerchant = sqlobj.update_Merchant_Subscription(objmerchant);

            //        _Merchant objcredit = new _Merchant();
            //        objcredit.Merchant_ID = Convert.ToInt32(Session["MerchantId"].ToString());
            //        SqlDataReader resultDR = objCredit.UpdateMerchantSuccessfulSubscriptionDateByID(objcredit);
            ...
            //        _Merchant objcredit = new _Merchant();
            //        objcredit.Merchant_ID = Convert.ToInt32(Session["MerchantId"].ToString());
            //        SqlDataReader resultDR = objCredit.UpdateMerchantSubscriptionFailsDateByID(objcredit);
            //        lblSubscriptionFailed.Visible = true;
            //    }

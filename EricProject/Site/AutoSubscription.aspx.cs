...
{
    public partial class AutoSubscription : System.Web.UI.Page
    {
        ...
        protected void btnRedeem_Click(object sender, EventArgs e)
        {
            ...
            if (result.Bank_Message == "Approved")
            {
                ...
                SqlDataReader drMerchant = sqlobj.GetMerchantDetailsById(objMerchant);
                while (drMerchant.Read())
                {
                    objMerchant.Merchant_ID = Convert.ToInt32(Session["MerchantId"].ToString());
                    SqlDataReader drCheckCampainsActiveInactiveByMerchantID = sqlobj.CheckCampainsActiveInactiveByMerchantID(objMerchant);
                    while (drCheckCampainsActiveInactiveByMerchantID.Read())
                    {
                        if (drCheckCampainsActiveInactiveByMerchantID["Exists"].ToString() == "1")
                        {
                            ...
                            if (transactionStatus)
                            {
                                ...
                                objMerchant.Merchant_ID = Convert.ToInt32(Session["MerchantId"].ToString());
                                sqlobj.UpdateMerchantSuccessfulSubscriptionDateByID(objMerchant);
                                ...
                                else
                                {
                                    ...
                                    objMerchant.Merchant_ID = Convert.ToInt32(Session["MerchantId"].ToString());
                                    SqlDataReader drGetTotalReferralsByMerchantID = sqlobj.GetTotalReferralsByMerchantID(objMerchant);
                                    string ReferralTime = "";
                            ...
                            else
                            {
                                ...
                                objMerchant.Merchant_ID = Convert.ToInt32(Session["MerchantId"].ToString());
                                sqlobj.UpdateMerchantSubscriptionFailsDateByID(objMerchant);

                                //update_Merchant_Subscription to 0
                                objMerchant.Merchant_ID = Convert.ToInt32(Session["MerchantId"].ToString());
                                objMerchant.status = 0;
                                sqlobj.update_Merchant_Subscription(objMerchant);
        ...
        public bool CreditCard()
        {
            ...
            if (DRCredits.Read())
            {
                ...
                EricProject.LiveCreditCard.TransactionResult result = ws.SendAndCommit(txn);
                if (result.Bank_Message == "Approved")
                {
                    return true;
                }
